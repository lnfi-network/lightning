            ...
            if (!args.asset_id) {
              return args.lnd.default.addInvoice(
                ...
                (err, response) => {
                  ...
                  if (!Buffer.isBuffer(response.payment_addr)) {
                    return cbk([
                      503,
                      "ExpectedPaymentAddressInCreateInvoiceResponse",
                    ]);
                  }
                  ...
                  if (!response.payment_request) {
                    return cbk([
                      503,
                      "ExpectedPaymentRequestForCreatedInvoice",
                    ]);
                  }
              ...
              return args.tpr.taproot_asset_channels.addInvoice(
                ...
                (err, response) => {
                  ...
                  const { invoice_result, accepted_buy_quote } = response;

                  if (!Buffer.isBuffer(invoice_result.payment_addr)) {
                    return cbk([
                      503,
                      "ExpectedPaymentAddressInCreateInvoiceResponse",
                    ]);
                  ...
                  if (!invoice_result.payment_request) {
                    return cbk([
                      503,
                      "ExpectedPaymentRequestForCreatedInvoice",
                    ]);
                  ...
                  try {
                    parsePaymentRequest({
                      request: invoice_result.payment_request,
                    });
                  } catch (err) {
                    return cbk([503, "ExpectedValidPaymentRequestForInvoice"]);
                  }

                  const payment = invoice_result.payment_addr.toString("hex");
                  const req = parsePaymentRequest({
                    request: invoice_result.payment_request,
                  });
                  ...
                  return cbk(null, {
                    ...
                    asset_id: hexAsBuffer(args.asset_id),
                    accepted_buy_quote: {
                      id: bufferAsHex(accepted_buy_quote.id),
                      peer: accepted_buy_quote.peer,
                      scid: accepted_buy_quote.scid,
                      asset_amount: accepted_buy_quote.asset_amount, //taprootAssetAmount
                      ask_price: accepted_buy_quote.ask_price, //msats
                      expiry: accepted_buy_quote.expiry,
                    },
                  });
                }
              );
